...
type TestCase = { id: string; provider: string; model: string; feature: string; label: string; costly: boolean };
type Result = { id: string; provider: string; feature: string; passed: boolean; latency_ms: number; metrics: Record<string, unknown>; error: string | null; diagnostics: RunDiagnostics | null; phoenix_url: string };
...
export default function Tests({ version, cases, availability, phoenixUrl }: { version: string; cases: TestCase[]; availability: Record<string, boolean>; phoenixUrl: string }) {
    ...
    return <div className="k-page"><Head title="Prism Lab Test Suite" /><KineticNav version={version} /><main className="mx-auto max-w-7xl px-6 py-16">
        ...
        {results.length > 0 && <section className="mt-6 grid gap-4">{results.map(result => <article key={result.id} className="k-card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><span className={result.passed ? 'text-[var(--k-cyan)]' : 'text-[var(--k-mag)]'}>{result.passed ? 'PASS' : 'FAIL'}</span><h2 className="mt-1 text-xl font-bold">{result.id}</h2></div><span className="k-mono">{result.latency_ms} ms</span></div>{result.error && <p className="mt-3 text-[var(--k-mag)]">{result.error}</p>}<RunDiagnosticsView diagnostics={result.diagnostics} /><pre className="mt-4 overflow-auto text-xs text-[var(--k-ink-2)]">{JSON.stringify(result.metrics, null, 2)}</pre><a href={result.phoenix_url} target="_blank" rel="noreferrer" className="k-mono mt-3 inline-block">Inspect Phoenix ↗</a></article>)}</section>}
